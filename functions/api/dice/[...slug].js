...
export async function onRequest({ request, env }) {
  ...
  if (!env.XBSKV) {
    return new Response('KV Binding "XBSKV" not found. Check your EdgeOne Pages configuration.', { status: 500 });
  }
  ...
  if (pathname.endsWith('/api/dice/log') && request.method === 'PUT') {
    try {
      ...
      await env.XBSKV.put(storageKey, JSON.stringify(generateStorageData(logdata, name)));
  ...
  if (pathname.endsWith('/api/dice/load_data') && request.method === 'GET') {
    try {
      ...
      const storageKey = `${key}#${password}`;
      const storedData = await env.XBSKV.get(storageKey);
